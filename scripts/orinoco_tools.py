...
# -*- coding: utf-8 -*-
"""
These tools are for using Orinoco software (https://github.com/simard-landscape-lab/orinoco)
...
def make_distance(water,ocean,folders,delta,pixel_step):
    ...
    with rasterio.open("%s_distance_%s.tif" %(folders[8]/delta,xres),'w',**p1) as dst:
    ...
    with rasterio.open('%s_segments_%sx%s.tif' %(folders[8]/delta,xres,pixel_step), 'w', **p2) as ds:
        ds.write(segments.astype(np.int32), 1)

    if os.path.isfile('%s_segments_%sx%s.shp' %(folders[7]/delta,xres,pixel_step)):
    ...
    # except: ''
    df_edges.to_file('%s_river_centerline_%sx%s.shp' %(folders[7]/delta,xres,pixel_step), driver='ESRI Shapefile')
    df_edges2 = df_edges.to_crs(4326)
    df_edges2.to_file('%s_river_centerline_%sx%s.geojson' %(folders[7]/delta,xres,pixel_step), driver='GeoJSON')
    ...
    df_nodes2.to_file('%s_river_centernode_%sx%s.geojson' %(folders[7]/delta,xres,pixel_step), driver='GeoJSON')
    ...
    df_nodes.to_file('%s_river_centernode_%sx%s.shp' %(folders[7]/delta,xres,pixel_step), driver='ESRI Shapefile')
    ...
    with rasterio.open("%s_widths_%sx%s.tif" %(folders[8]/delta,xres,pixel_step), 'w', **p) as ds:
