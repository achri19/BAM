...
if os.getcwd().split('/')[1] == 'Users':
    code_path = '/users/alchrist/documents/github/BAM/processing/code/'
else:
...
def make_segments_for_swot(folders,delta,ref,parameters,skip):#,min_size,scale):
    ...
    if skip == False:
        ...
        river = rasterio.open('%s_rivers_%s.tif' %(folders[8]/delta,xres))
        ...
        ix = [0.001,0.005,1]#compactness
        seg_size = [50000,500000,5000000,10000000]
        ...
        print('Target Number of Segments: %s' %(jx))
        for i in range(0,len(ix)):
            for j in range(2,len(jx)):
                compactness = ix[i]
                nsegments = jx[j]
                if os.path.isfile('%s_oceandem_superpixels_%sm2_compact%s_segs%s_slic.shp' %(segments_dir/delta,seg_size[j],compactness,nsegments))==False:
                    ...
                    try:os.remove('%s_oceandem_superpixels_%sm2_compact%s_segs%s_slic.shp'%(segments_dir/delta,seg_size[j],compactness,nsegments))
                    except:''
                    os.system('gdal_polygonize.py -q %s_oceandem_superpixels_%sm2_compact%s_segs%s_slic.tif '\
                                '%s_oceandem_superpixels_%sm2_compact%s_segs%s_slic.shp '\
                ...
                axes[i,j].set_title('compact = %s #Segments = %s' %(ix[i],jx[j]),fontsize=18)
                fig.suptitle('Slic Segmentation', fontsize=30)
        ...
        seg_size = [5000,100000,1000000,10000000] # to make segments of minimum size 0.1ha, 1ha, 10ha, 100ha
        ...
        print('Scale: %s' %(ix))
        print('Sigma: %s' %(jx))
        print('Minimum size (#pixels): %s' %(zx))
        ...
        for j in range(0,len(jx)):
            ...
            for i in range(0,len(ix)):
                ...
                for z in range(0,len(zx)):
                        min_size = zx[z]
                        if os.path.isfile('%s_oceandem_superpixels_%sm2_scale%s_sigma%s_min%s_felz.shp' %(segments_dir/delta,seg_size[z],scale,sigma,min_size))==False:
                            print('Scale: %s     Sigma: %s     Min Size (#pixels): %s' %(scale, sigma, min_size))
                            ...
                            try:
                                os.remove('%s_oceandem_superpixels_%sm2_scale%s_sigma%s_min%s_felz.shp' \
                                            %(segments_dir/delta,seg_size[z],scale,sigma,min_size))
                            except:''
                            os.system('gdal_polygonize.py %s_oceandem_superpixels_%sm2_scale%s_sigma%s_min%s_felz.tif '\
                                        '%s_oceandem_superpixels_%sm2_scale%s_sigma%s_min%s_felz.shp -q'\
                        ...
                        axes[i,z].set_title('scale = %s min #pixels = %s' %(ix[i],zx[z]),fontsize=18)
                        fig.suptitle('Felz Segmentation with Sigma = %s' %(jx[j]),fontsize=30)

            plt.tight_layout()
            plt.savefig('%s_Segments_Ocean_felz_%s.png' %(segments_dir/delta,sigma))
        ...
        for p in range(len(px)):
            ...
            if os.path.isfile('%s_river_superpixels_%sx%s.shp -q' %(segments_dir/delta,xres,pixel_step)) == False:
                ...
                with rasterio.open('%s_river_superpixels_%sx%s.tif' %(segments_dir/delta,xres,pixel_step), 'w', **save_profile) as ds:
                    ds.write(segments.astype(np.int32), 1)

                if os.path.isfile('%s_river_superpixels_%sx%s.shp' %(segments_dir/delta,xres,pixel_step)):
                    os.remove('%s_river_superpixels_%sx%s.shp' %(segments_dir/delta,xres,pixel_step))

                ## Polygonize the segments
                print('\n######[Make_Channel_Networks][Orinoco --> segment raster to shapefile] .......\n')
                os.system('gdal_polygonize.py %s_river_superpixels_%sx%s.tif %s_river_superpixels_%sx%s.shp -q' %(segments_dir/delta,xres,pixel_step,segments_dir/delta,xres,pixel_step))
                #segments = gpd.read_file('%s%s_segments_%sx%s.shp'%(folders[7],delta,xres,pixel_step))
            else:
                segments = rasterio.open('%s_river_superpixels_%sx%s.tif' %(segments_dir/delta,xres,pixel_step)).read(1)
            axes[p].imshow(segments,cmap=cmap)
            axes[p].set_title('pixel steps = %s x 30m' %(px[p]),fontsize=30)


        plt.tight_layout()
        plt.savefig('%s_Segments_River.png' %(segments_dir/delta))
