...
pip install -q --target=/content/drive/MyDrive/installations nose rtree shapely numpy==1.23 pandas~=1.5.3 ipython~=7.34.0 ipykernel~=5.5.6 decorator<5.0,>=4.0.2 mpi4py triangle dill Pmw pymetis geemap cmocean geopandas netCDF4 pyproj fiona pygeos rasterio rasterstats scikit-fmm scipy backports.zoneinfo > /dev/null 2>&1


echo "(3) Install gdal"
apt-get -y install --target=/content/drive/MyDrive/installations python-gdal gdal-bin python3-gdal #> /dev/null 2>&1
