...
echo "nose mpi4py triangle dill Pmw pymetis anuga mpi4py gdal netcdf4 geemap cmocean geopandas fiona pygeos rasterio rasterstats scikit-fmm rtree pyTMD Orinoco"
pip install -q --target=/content/drive/MyDrive/installations nose mpi4py triangle dill Pmw pymetis mpi4py netcdf4 geemap cmocean geopandas fiona pygeos rasterio rasterstats scikit-fmm rtree backports.zoneinfo pyTMD git+https://github.com/simard-landscape-lab/orinoco.git
#> /dev/null 2>&1
...
apt-get -q -y install --target=/content/drive/MyDrive/installations libgdal-dev
