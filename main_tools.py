...
def make_polygons(delta,folders,parameters,ref,watermaskname,templates_path,skip=False):
    ...
    if skip == False:
        if os.path.isfile('%s%s_landmask_%s.shp' %(folders[7],delta,xres))==False:
        ...
        try:lakes = gpd.read_file("%s%s_lakes_%s.shp" %(folders[7],delta,xres))
        except:
            try:
        ...
        try:oceans = gpd.read_file("%s%s_fulloceans_%s.shp" %(folders[7],delta,xres))
        except:
        ...
        try: ocean_lake = gpd.read_file("%s%s_ocean_and_lake_%s.shp" %(folders[7],delta,xres))
        except:
        ...
        try:rivers = gpd.read_file("%s%s_rivers_%s.shp" %(folders[7],delta,xres))
        except:
        ...
        try: allwater = gpd.read_file("%s%s_water_connected_and_lakes_%s.shp" %(folders[7],delta,xres))
        except:
        ...
        try:lands = gpd.read_file("%s%s_lands_%s.shp" %(folders[7],delta,xres))
        except:
...
def make_channel_networks(folders,delta,ref,parameters,pixel_step,skip=False):
    ...
    distance = gdal.Open('%s%s_distance_%s.tif' %(folders[8],delta,xres)).ReadAsArray()
